...
contract Ponte is Ownable{
    ...
    function createGroup(uint256 _groupId, bytes32 _schemaId, address _attester, string calldata _uri) external onlyOwner{ 
    ...
    function joinGroup(uint256 _identityCommitment, uint256 _groupId, bytes32 _uid) external {
        ...
        require(_attestation.attester == _group.attester,"This attestation is not approved by official attester/s");
        // check if msg.sender is recipient
        require(_attestation.recipient == msg.sender, "You are not owner of this attestation");
        semaphore.addMember(_groupId, _identityCommitment);
    }

    function removeMember(bytes32 _uid, uint256 _groupId, uint256 _identityCommitment, uint256[] calldata _proofSiblings, uint8[] calldata _proofPathIndices) external { 
        Attestation memory _attestation = eas.getAttestation(_uid);
        require(_attestation.revocationTime != 0, "Attestation is not revoked"); // if attestation is revoked, removeMember
        semaphore.removeMember(_groupId, _identityCommitment, _proofSiblings, _proofPathIndices); // anyone can do this if attestation is revoked 
    }

    function registerNewSchema(string calldata _schema, address _resolverAddress, bool _revocable) external onlyOwner{ // add modifier potentially
    ...
    function attest(AttestationRequest calldata _request) external payable onlyOwner{
        eas.attest(_request);

    }

    function revoke(RevocationRequest calldata _request) external{ // no need for modifiers, revoke itself is checking who's revoking the attestation 
        eas.revoke(_request);
    }
    ...
    function sendMessage(
        ...
        verifier.verifyProof(merkleTreeRoot, nullifierHash, message, _groupId, proof, merkleTreeDepth);
